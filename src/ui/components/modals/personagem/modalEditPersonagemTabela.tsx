...
export default function ModalEditPersonagemTabela() {
  const { isOpen, data, onClose } = useTabelaPersonagemModal();
  ...
  const salvar = async (e: React.FormEvent) => {
    ...
    const lista = (await localforage.getItem<Personagem[]>("personagensNaAventura")) ?? [];
    const atualizada = lista.map((p) => (p.id === form.id ? form : p));
    await localforage.setItem("personagensNaAventura", atualizada);
